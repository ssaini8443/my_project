...
var addCartBtn = document.querySelectorAll('.add-cart-btn');
addCartBtn.forEach(function (btn) {
  btn.addEventListener('click', function () {
    var product = JSON.parse(btn.dataset.product);
    axios.post('/cart/add', product).then(function (res) {
...
var successAlert = document.querySelector('#order-alert');
...
function renderAdminOrders() {
  return _renderAdminOrders.apply(this, arguments);
}

function _renderAdminOrders() {
  _renderAdminOrders = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var adminTBody, orders, adminTBodyHtml, renderItems, generateTbodyHtml;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            generateTbodyHtml = function _generateTbodyHtml(orders) {
              return orders.map(function (order) {
                return "\n                <tr>\n                <td class=\"border px-4 py-2 text-green-900\">\n                    <p>".concat(order._id, "</p>\n                    <div>").concat(renderItems(order.products), "</div>\n                </td>\n                <td class=\"border px-4 py-2\">").concat(order.userId.name, "</td>\n                <td class=\"border px-4 py-2\">").concat(order.address, "</td>\n                <td class=\"border px-4 py-2\">\n                    <div class=\"inline-block relative w-64\">\n                        <form action=\"/admin/orders/status\" method=\"POST\">\n                            <input type=\"hidden\" name=\"orderId\" value=\"").concat(order._id, "\">\n                            <select name=\"status\" onchange=\"this.form.submit()\"\n                                class=\"block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline\">\n                                <option value=\"order_placed\"\n                                    ").concat(order.orderStatus === 'order_placed' ? 'selected' : '', ">\n                                    Placed</option>\n                                <option value=\"confirmed\" ").concat(order.orderStatus === 'confirmed' ? 'selected' : '', ">\n                                    Confirmed</option>\n                                <option value=\"prepared\" ").concat(order.orderStatus === 'prepared' ? 'selected' : '', ">\n                                    Prepared</option>\n                                <option value=\"delivered\" ").concat(order.orderStatus === 'delivered' ? 'selected' : '', ">\n                                    Delivered\n                                </option>\n                                <option value=\"completed\" ").concat(order.orderStatus === 'completed' ? 'selected' : '', ">\n                                    Completed\n                                </option>\n                            </select>\n                        </form>\n                        <div\n                            class=\"pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700\">\n                            <svg class=\"fill-current h-4 w-4\" xmlns=\"http://www.w3.org/2000/svg\"\n                                viewBox=\"0 0 20 20\">\n                                <path\n                                    d=\"M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z\" />\n                            </svg>\n                        </div>\n                    </div>\n                </td>\n                <td class=\"border px-4 py-2\">\n                ").concat(moment(order.timestamp).format('hh:mm A'), "\n                </td>\n                <td class=\"border px-4 py-2\">\n                ").concat(order.paymentType, "\n                </td>\n            </tr>\n        ");
              }).join('');
            };

            renderItems = function _renderItems(items) {
              var parsedItems = Object.values(items);
              return parsedItems.map(function (menuItem) {
                return "\n                    <p>".concat(menuItem.cartproduct.productName, " - ").concat(menuItem.quantity, " pcs </p>\n                ");
              }).join('');
            };

            adminTBody = document.querySelector('#admin-order-tbody');
            orders = [];
            axios.get('/admin/orders', {
              ...
              adminTBody.innerHTML = adminTBodyHtml; // <div>${renderItems(order.items)}</div>
            })["catch"](function (err) {
              console.log(err);
            });
...
renderAdminOrders();
var typesOfStatus = document.getElementsByClassName('types-of-status');
var currentOrder = document.getElementById('currentorder') ? document.getElementById('currentorder').value : null;

if (currentOrder) {
  currentOrder = JSON.parse(currentOrder); //types-of-status
}

function changeOrderStatus(corder) {
  var statusCompleted = true;
  Array.from(typesOfStatus).forEach(function (type) {
    var prop = type.dataset.statusype;
...
if (payMode) {
  payMode.addEventListener('change', function (e) {
...
if (placeOrderForm) {
  ...
  placeOrderForm.addEventListener('submit', function (e) {
    e.preventDefault();
    var fData = new FormData(placeOrderForm);
    var formObj = {};

    var _iterator = _createForOfIteratorHelper(fData.entries()),
        _step;
    ...
    if (!card) {
      axios.post('/order', formObj).then(function (res) {
        console.log(res);
...
function updateCartCounter(count) {
  var cartCounterIcon = document.getElementById('cart-counter-icon'); // let cartCounterIconValue = parseInt(cartCounterIcon.innerText);
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkcapstoneproject"] = self["webpackChunkcapstoneproject"] || [];
